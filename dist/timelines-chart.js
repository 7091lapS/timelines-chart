  ...
  var timelines = Kapsule({
    ...
    init: function init(el, state) {
      ...
      function addTooltips() {
        ...
        state.segmentTooltip = d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (d) {
          ...
          return '<div class="labelval-title">' + d.labelVal + '</div>' + '<br>' // + (normVal?' (<strong>' + Math.round((d.val-state.zColorScale.domain()[0])/normVal*100*100)/100 + '%</strong>)':'') + '<br>'
          + "".concat(dateFormat(d.timeRange[0]), " - ").concat(dateFormat(d.timeRange[1]));
        });
    ...
    update: function update(state) {
      ...
      function setupDimensions() {
        ...
        state.graph.attr('transform', "translate(".concat(state.leftMargin + state.labelsGWidth, ",").concat(state.topMargin, ")"));
      ...
      function renderAxises() {
        ...
        state.svg.select('.axises').attr('transform', "translate(".concat(state.leftMargin + state.labelsGWidth, ",").concat(state.topMargin, ")")); // X

        state.xAxis.scale(state.xScale) // .ticks(state.isMobile ? 4 : Math.round(state.graphW*0.0011))
        .ticks(state.isMobile ? 4 : null).tickFormat(state.xTickFormat);
        state.xGrid.scale(state.xScale).ticks(state.isMobile ? 4 : state.xAxis.ticks()[0]).tickFormat('');
        state.svg.select('g.x-axis').style('stroke-opacity', 0).style('fill-opacity', 0).attr('transform', "translate(0,".concat(-state.topMargin, ")")).transition().duration(state.transDuration).call(state.xAxis).style('stroke-opacity', 1).style('fill-opacity', 1);
        ...
        state.svg.select('g.y-axis').transition().duration(state.transDuration) // .attr('transform', 'translate(' + state.graphW + ', 0)')
        .style('font-size', fontSize + 'px').call(state.yAxis); // Grp
        ...
        state.svg.select('.y-axis').attr('transform', "translate(".concat(-(state.leftMargin + state.labelsGWidth), ",0)"));
